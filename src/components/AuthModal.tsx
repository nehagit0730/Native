...
const DEFAULT_ACCOUNTS = [
  {
    ...
    title: 'Home Buyer / Tenant Account'
  },
  {
    role: 'owner' as Role,
    ...
    title: 'Property Owner / Seller Account'
  },
  {
    role: 'broker' as Role,
    ...
    title: 'Agent & Broker Portal Account'
  },
  {
    ...
    title: 'RERA Builder & Developer Account'
  }
];
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleAdminLogin = (e: React.FormEvent) => {
    ...
    if (adminUsername.trim() === 'admin' && adminPassword.trim() === 'admin') {
      setAdminError('');
      onAuthenticated('admin', 'admin@shinenative.com', 'System Admin', 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=crop&w=150&q=80');
      onClose();
    } else {
  ...
  const handleRealFirebaseGoogle = async () => {
    ...
    try {
      ...
      const email = fbUser.email || `${activeRole}_user@gmail.com`;
      const name = fbUser.displayName || customName.trim() || `Verified ${activeRole.toUpperCase()}`;
      ...
      // Fallback if popup blocked in preview iframe or popup closed
      if (err?.code === 'auth/popup-blocked' || err?.code === 'auth/cancelled-popup-request' || err?.message?.includes('popup')) {
        // Graceful fallback to instant Google SSO mode
        handleQuickGoogleAuth(customEmail || `${activeRole}_user@gmail.com`, customName || `Verified ${activeRole}`);
      } else {
        setAuthError(err.message || 'Firebase authentication failed. Please try again.');
        setIsSigningIn(false);
        setOauthStep('form');
      }
    }
  ...
  // Quick Google SSO account choice
  const handleQuickGoogleAuth = (selectedEmail?: string, selectedName?: string, selectedAvatar?: string) => {
    setOauthStep('connecting');
    setIsSigningIn(true);

    const email = selectedEmail || customEmail.trim() || `${activeRole}_user@gmail.com`;
    const defaultAccount = DEFAULT_ACCOUNTS.find(a => a.role === activeRole);
    const name = selectedName || customName.trim() || (defaultAccount ? defaultAccount.name : `Verified ${activeRole}`);
    const picture = selectedAvatar || (defaultAccount ? defaultAccount.picture : 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=150&q=80');
  ...
  const currentDefaultAcc = DEFAULT_ACCOUNTS.find(a => a.role === activeRole);
              ...
              {oauthStep === 'form' && (
                <div className="space-y-5">
                  ...
                  >
                    ...
                    <span>
                      {authMode === 'signup' ? 'Sign Up with Real Google Account' : 'Log In with Real Google Account'}
                    </span>
                  </button>

                  {/* 1-Click Quick Google Account Choice */}
                  {currentDefaultAcc && (
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3">
                      <div className="flex items-center justify-between text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                        <span>Or Quick Select Preconfigured Account</span>
                        <span className="text-emerald-600 flex items-center">
                          <Check className="w-3 h-3 mr-0.5" /> Demo SSO
                        </span>
                      </div>
                      ...
                      <div className="flex items-center justify-between bg-white p-3 rounded-xl border border-slate-200 shadow-2xs">
                        <div className="flex items-center space-x-3">
                          <img
                            src={currentDefaultAcc.picture}
                            alt={currentDefaultAcc.name}
                            className="w-10 h-10 rounded-full object-cover border-2 border-blue-500"
                          />
                          <div className="text-left">
                            <h5 className="font-extrabold text-xs text-slate-900">{currentDefaultAcc.name}</h5>
                            <p className="text-[11px] text-slate-500 font-medium">{currentDefaultAcc.email}</p>
                          </div>
                        </div>

                        <button
                          onClick={() => handleQuickGoogleAuth(currentDefaultAcc.email, currentDefaultAcc.name, currentDefaultAcc.picture)}
                          className="bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs py-2 px-3 rounded-lg shadow-sm flex items-center space-x-1 cursor-pointer transition-all"
                        >
                  ...
                  <div className="relative text-center">
                    ...
                    <span className="relative bg-white px-3 text-[10px] font-bold uppercase text-slate-400">
                      Or custom Gmail
                    </span>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-700">Gmail / Google Account</label>
                      <input
                        type="email"
                        placeholder={`e.g. yourname.${activeRole}@gmail.com`}
                        value={customEmail}
                    ...
                    <button
                      ...
                      className="w-full bg-white hover:bg-slate-50 border border-slate-300 text-slate-800 font-extrabold py-3 px-4 rounded-xl shadow-md text-xs flex items-center justify-center space-x-3 transition-all cursor-pointer hover:border-slate-400"
                    >
                      <svg className="w-5 h-5" viewBox="0 0 24 24">
                        <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                        <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                      ...
                      <span>
                        {authMode === 'signup' ? 'Continue Custom Google Sign Up' : 'Continue Custom Google Login'}
                      </span>
                    </button>
                  </div>
                </div>
