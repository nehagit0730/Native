...
export const PropertyDetail: React.FC<PropertyDetailProps> = ({
  ...
  const shareUrl = typeof window !== 'undefined' ? `${window.location.origin}/properties/${property.slug || property.id}` : '';
  ...
  return (
    <div className={isFullPage ? "min-h-screen bg-slate-50 pb-16 animate-in fade-in duration-200" : "fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200"}>
      <div className={isFullPage ? "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6" : "bg-white w-full max-w-5xl rounded-3xl shadow-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[95vh]"}>
        {/* Full Page Breadcrumbs & Action Bar */}
        {isFullPage && (
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:px-6 rounded-2xl shadow-sm border border-slate-200">
            <div className="flex items-center space-x-2 text-xs font-semibold text-slate-500 overflow-x-auto">
              <button onClick={onClose} className="hover:text-blue-600 flex items-center cursor-pointer">
                Home
              </button>
              <span>/</span>
              <button onClick={onClose} className="hover:text-blue-600 cursor-pointer">
                Properties
              </button>
              <span>/</span>
              <span className="text-slate-500 font-medium">{property.city}</span>
              <span>/</span>
              <span className="text-slate-900 font-extrabold truncate max-w-xs">{property.title}</span>
            </div>

            <div className="flex items-center space-x-3 shrink-0">
              <button
                onClick={onClose}
                className="bg-slate-100 hover:bg-slate-200 text-slate-800 font-extrabold text-xs px-4 py-2 rounded-xl transition-all cursor-pointer flex items-center space-x-1.5 border border-slate-200"
              >
                <span>← Back to Listings</span>
              </button>
              <button
                onClick={() => setShowShareModal(true)}
                className="bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs px-4 py-2 rounded-xl transition-all cursor-pointer flex items-center space-x-1.5 shadow-md"
              >
        ...
        {/* Modal Header Bar (only in popup mode) */}
        {!isFullPage && (
        ...
        <div className={isFullPage ? "bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-8" : "p-4 sm:p-6 overflow-y-auto flex-1 space-y-8"}>
